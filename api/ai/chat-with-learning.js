/**
 * 🧠 API de Chat con Sistema de Aprendizaje Integrado
 * Versión mejorada que captura interacciones automáticamente
 */
...
async function captureInteraction({
  ...
  if (!supabaseAdmin) {
    console.warn('[chat-with-learning] Supabase not configured, skipping capture');
    return null;
  }
  ...
  try {
    ...
    if (error) {
      console.error('[chat-with-learning] Error capturing interaction:', error);
      return null;
    }

    return data?.id || null;
  } catch (error) {
    console.error('[chat-with-learning] Unexpected error capturing interaction:', error);
    return null;
  }
}
...
async function classifyIntent(prompt) {
  ...
  try {
    // Obtener todos los intents con sus ejemplos
    ...
    for (const intent of intents) {
      ...
      // Verificar si el prompt contiene palabras clave del intent
    ...
    console.error('[chat-with-learning] Error classifying intent:', error);
    return null;
  }
}

/**
 * Endpoint mejorado con captura automática
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    const {
      ...
      capture_interaction = true, // Por defecto capturar
    } = req.body;
    ...
    // Extraer prompt del último mensaje
    ...
    let tokensUsed = 0;
    let modelUsed = model;
    let interactionId = null; // Para devolver el ID de la interacción capturada
    ...
    switch (provider.toLowerCase()) {
      case 'deepseek':
        ...
        if (capture_interaction) {
          ...
          const capturedId = await captureInteraction({
            userId: user?.id || null,
            sessionId,
            prompt,
            response: content,
        ...
        return res.status(200).json({
          ...
          interaction_id: interactionId || null, // Devolver ID de interacción
        });
    ...
    console.error('[api/ai/chat-with-learning] Error:', error);
