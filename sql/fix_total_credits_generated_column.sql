-- ========================================
-- 🔧 CORRECCIÓN: consume_credits - total_credits es GENERATED
-- ========================================
-- PROBLEMA: total_credits es una columna GENERATED ALWAYS AS (monthly + purchased + bonus) STORED
-- SOLUCIÓN: NO intentar actualizar total_credits directamente, se calcula automáticamente
-- ========================================
...
BEGIN
  ...
  -- Consumir primero de monthly_credits (expiran)
  ...
  -- Luego de bonus_credits (créditos promocionales/de prueba - SÍ se pueden usar)
  ...
  -- Finalmente de purchased_credits (no expiran)
  IF v_remaining > 0 THEN
    v_purchased_used := v_remaining;
  END IF;

  -- 6. Actualizar créditos
  -- ⚠️ IMPORTANTE: total_credits es GENERATED, se calcula automáticamente
  -- NO intentar actualizarlo directamente - solo actualizar monthly, purchased, bonus
  UPDATE user_credits
  SET
...
SELECT 
  '✅ Función consume_credits corregida' as status,
  routine_name,
  routine_type
