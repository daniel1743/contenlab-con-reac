...
function InputForm({ onGenerate, loading }) {
  ...
  return (
    <form className="seo-input-form" onSubmit={handleSubmit}>
      <div className="seo-form-row">
        <div className="seo-form-group">
          <label className="seo-form-label" htmlFor="domain">Dominio</label>
          <input
            id="domain"
            type="text"
        ...
        <div className="seo-form-group">
          <label className="seo-form-label" htmlFor="period">Período de análisis</label>
          <select
            id="period"
            className="seo-form-select"
            value={period}
      ...
      <button type="submit" className="seo-btn seo-btn-primary" disabled={loading}>
...
function LoadingSpinner() {
  return (
    <div className="seo-loading-spinner">
...
function DonutInfographic({ data }) {
  ...
  return (
    <div className="seo-infographic-card seo-fade-in">
      <div className="seo-infographic-header">
        <h3 className="seo-infographic-title">Donut Multi-Anillo</h3>
        <div className="seo-export-buttons">
          <button className="seo-btn seo-btn-secondary" onClick={exportSVG}>SVG</button>
          <button className="seo-btn seo-btn-secondary" onClick={exportPNG}>PNG</button>
        </div>
      </div>
      <p className="seo-infographic-description">
      ...
      <div className="seo-svg-container">
        <svg ref={svgRef} width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
          ...
          {/* Center health score */}
          <text x={centerX} y={centerY - 10} textAnchor="middle" className="seo-svg-health-score" fill="var(--color-primary)">
            {formatPercent(data.health, 1)}
          </text>
          <text x={centerX} y={centerY + 20} textAnchor="middle" className="seo-svg-label" fill="var(--color-text-secondary)">
            Health Score
          </text>

          {/* Site title */}
          <text x={centerX} y={30} textAnchor="middle" className="seo-svg-title" fill="var(--color-text)">
            {data.site}
          </text>
        </svg>
        ...
        {tooltip && (
          ...
          >
            {tooltip.type === 'cluster' ? (
              <>
                <div className="seo-tooltip-title">{tooltip.item.name}</div>
                <div className="seo-tooltip-value">Tráfico: {formatNumber(tooltip.item.traffic)}</div>
                <div className="seo-tooltip-value">Conversión: {formatPercent(tooltip.item.conversionRate)}%</div>
                <div className="seo-tooltip-value">Score: {tooltip.item.contentScore}/100</div>
                <div className="seo-tooltip-value">Participación: {formatPercent(tooltip.item.share)}%</div>
              </>
            ) : (
              <>
                <div className="seo-tooltip-title">{tooltip.item.label}</div>
                <div className="seo-tooltip-value">
                  Valor: {formatNumber(tooltip.item.value)}{tooltip.item.unit || ''}
                </div>
                {tooltip.item.change !== null && (
                  <div className={`seo-tooltip-change ${tooltip.item.change >= 0 ? 'positive' : 'negative'}`}>
...
function WheelInfographic({ data }) {
  ...
  return (
    <div className="seo-infographic-card seo-fade-in">
      <div className="seo-infographic-header">
        <h3 className="seo-infographic-title">Rueda 8 Pasos</h3>
        <div className="seo-export-buttons">
          <button className="seo-btn seo-btn-secondary" onClick={exportSVG}>SVG</button>
          <button className="seo-btn seo-btn-secondary" onClick={exportPNG}>PNG</button>
        </div>
      ...
      <div className="seo-svg-container">
        <svg ref={svgRef} width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
          ...
          <text x={centerX} y={centerY - 5} textAnchor="middle" className="seo-svg-title" fill="var(--color-text)" fontSize="16">
            Pipeline SEO
          </text>
          <text x={centerX} y={centerY + 15} textAnchor="middle" className="seo-svg-label" fill="var(--color-text-secondary)" fontSize="12">
            Programático
          </text>
        </svg>
        ...
        {tooltip && (
          ...
          >
            <div className="seo-tooltip-title">{tooltip.step.name}</div>
            <div className="seo-tooltip-value">Progreso: {formatPercent(tooltip.step.status * 100)}%</div>
            <div className="seo-tooltip-value">Nota: {tooltip.step.notes}</div>
          </div>
        )}
      </div>
    </div>
...
function PuzzleInfographic({ data }) {
  ...
  return (
    <div className="seo-infographic-card seo-fade-in">
      <div className="seo-infographic-header">
        <h3 className="seo-infographic-title">Pastel Rompecabezas</h3>
        <div className="seo-export-buttons">
          <button className="seo-btn seo-btn-secondary" onClick={exportSVG}>SVG</button>
          <button className="seo-btn seo-btn-secondary" onClick={exportPNG}>PNG</button>
        </div>
      ...
      <div className="seo-svg-container">
        <svg ref={svgRef} width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
          ...
          <text x={centerX} y={centerY - 5} textAnchor="middle" className="seo-svg-title" fill="var(--color-text)" fontSize="14">
            Clusters
          </text>
          <text x={centerX} y={centerY + 15} textAnchor="middle" className="seo-svg-label" fill="var(--color-text-secondary)" fontSize="12">
            Temáticos
        ...
        {tooltip && (
          ...
          >
            <div className="seo-tooltip-title">{tooltip.cluster.name}</div>
            <div className="seo-tooltip-value">Tráfico: {formatNumber(tooltip.cluster.traffic)}</div>
            <div className="seo-tooltip-value">Participación: {formatPercent(tooltip.cluster.share)}%</div>
            <div className="seo-tooltip-value">Conversión: {formatPercent(tooltip.cluster.conversionRate)}%</div>
            <div className="seo-tooltip-value">Content Score: {tooltip.cluster.contentScore}/100</div>
          </div>
        )}
      </div>
...
export default function SEOInfographicsContainer() {
  ...
  return (
    <div className="seo-infographics-container">
      <div className="seo-header">
        <h2 style={{ fontSize: '24px', fontWeight: '600', color: 'var(--color-text)' }}>SEO Analytics - Infografías Automáticas</h2>
      </div>
